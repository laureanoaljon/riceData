...
class Productionmodel extends CI_Model {
    ...
    function get_annual_production($psgc_code, $year, $ecosystem){
    ...
    function checkOrGet_production_total_current($location_code, $location_type,$year, $ecosystem, $year_type, $period_unit){
        $this->db->select('l.locName AS location_name, k.year AS year, SUM("k"."estProduction") as value, k.yearType, k.periodUnit');
    ...
    function get_production_total_current($location_code, $location_type,$year, $ecosystem){
        
        
        $data_array = $this->checkOrGet_production_total_current($location_code, $location_type, $year, $ecosystem, '3', 4);

        if (!empty($data_array)) {
            // $data_array has data
            return $this->get_production_total($location_code, $location_type, $year, $ecosystem);
        } else {
            // $data_array is empty
            $data_array = $this->checkOrGet_production_total_current($location_code, $location_type, $year, $ecosystem, '3', 3);
             if (!empty($data_array)) {
                // $data_array has data
                return $data_array[0];
            }else{
                $data_array = $this->checkOrGet_production_total_current($location_code, $location_type, $year, $ecosystem, '3', 2);
                if (!empty($data_array)) {
                    $data_array = $this->checkOrGet_production_total_current($location_code, $location_type, $year, $ecosystem, '2', 1);
                    return $data_array[0];
                }else{
                    $data_array = $this->checkOrGet_production_total_current($location_code, $location_type, $year, $ecosystem, '3', 1);
                    if (!empty($data_array)) {
                        // $data_array has data
                        return $data_array[0];
                    }else{
    ...
    function get_production_quarter($location_code, $location_type, $year, $ecosystem, $year_type){
        
        $this->db->select('l.locName AS location_name, k.year AS year, k.periodUnit AS period_unit, SUM("k"."estProduction") as value');
        $this->db->from('kpi_pays k');
        $this->db->join('ids_location l', 'k.locCode = l.locCode AND k.locType = l.locType');
        $this->db->join('ids_ecosystem e', 'k.eco = e.eco');
        $this->db->where('k.locCode', $location_code);
    ...
    function get_production_totals($location_code = null, $location_type,  $year_start, $year_end = NULL, 
        ...
        if($year_type == 1){
            ...
            $this->db->from('kpi_pays k'); // Get pay data when there is Period
        }
    ...
    function get_production_totals_geocodes($location_code = null, $location_type,  $year_start, $year_end = NULL, 
        ...
        $this->db->select('l.locName AS location_name, g.geoCode AS map_ID, k.year AS year, SUM("k"."estProduction") as value, k.psgc_code AS psgcCode');
        if($year_type == 1){
            $this->db->from('kpi_pay k');
        } else {
            $this->db->from('kpi_pays k');
    ...
    function get_production_totals_municities_all($location_code, $location_type,  $year_start = NULL, $year_end = NULL, 
                                                  $ecosystem, $order = NULL, $limit = NULL, $region_code = NULL){
        // Subquery
        $this->db->select('locCode, locType, locName');
        $this->db->from("ids_location");
        $this->db->where('locType', '2');
        $this->db->group_by(array("locCode", "locType", "locName"));
        $this->db->order_by('locCode', 'ASC');
        $subQuery =  $this->db->get_compiled_select();

        $this->db->select('l.locName AS location_name, t.locName AS province_name, g.geoCode AS map_ID, k.year AS year, k.estProduction as value');
        $this->db->from('ids_location l');
        $this->db->join("($subQuery) t", 'l.parent = t.locCode');
        $this->db->join('kpi_pay_prism k', 'l.locCode = k.locCode  AND l.parent = k.provinceCode', 'left outer');
        $this->db->join('ids_geocodes g', 'l.locCode = g.locCode AND l.parent = g.parent');
        $this->db->where_in('l.locCode', $location_code);
        $this->db->where('l.locType', '3');
        $this->db->where('k.sem', '3');
        $this->db->where('k.locType', '3');
        $this->db->where('k.year', $year_end);
        $this->db->where('g.locType', '3');
        $this->db->where('k.estProduction >', '0');
        $this->db->order_by('value', $order);
    ...
    function get_production_aggregate_prism ($location_code = NULL, $location_type, $year_start = NULL, $year_end = NULL, $aggregate_function = NULL, $ecosystem, $order = NULL, $limit = NULL, $region_code = NULL){
        ...
        if($aggregate_function == "MIN") {
            $this->db->select_min("k.estProduction");
        }
        elseif($aggregate_function == "MAX") {
            $this->db->select_max("k.estProduction");
        }
        elseif($aggregate_function == "MEDIAN") {
            $this->db->select('PERCENTILE_CONT(0.5) WITHIN GROUP(ORDER BY "k"."estProduction")');             
        }
    ...
    function get_production_totals_regions($location_code = null, $location_type,  $year_start, $year_end = NULL, 
                                        $ecosystem, $period = NULL, $year_type = NULL, $order = NULL, $limit = NULL, $region_code = NULL){
    ...
    function get_production_totals_municities($location_code, $location_type,  $year_start, $year_end = NULL, 
        ...
        $this->db->select("concat(\"munName\",'\\n',\"provName\") AS location_name, g.geoCode AS map_ID, k.year AS year, k.estProduction as value");
